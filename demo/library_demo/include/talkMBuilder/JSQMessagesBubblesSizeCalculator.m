                               ...
                               withLayout:(JSQMessagesCollectionViewFlowLayout *)layout
{
    ...
    if ([messageData isMediaMessage]) {
        ...
        //change to user default
//        ShareData *sdata = [ShareData ShareDataAction];
//        sdata.bubbleMaxWidth = [NSString stringWithFormat:@"%f", [self textBubbleWidthForLayout:layout]] ;   //talkmdev - bubbleMaxWidth
        
        [libsdk storeUserDefaultBubbleMaxWidth:[NSString stringWithFormat:@"%f", [self textBubbleWidthForLayout:layout]]];
        
        
        
//        NSLog(@"maximumTextWidth : %f", maximumTextWidth);
//        NSLog(@"messageData : %@", messageData);
//        CGRect stringRect = [[messageData text] boundingRectWithSize:CGSizeMake(maximumTextWidth, CGFLOAT_MAX)
//                                                             options:(NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading)
//                                                          attributes:@{ NSFontAttributeName : layout.messageBubbleFont }
//                                                             context:nil];  //origin 20171129
        
        NSString *tempMessageDataText = [NSString stringWithFormat:@"%@%@", [messageData text], @"12:12 AM"];
        ...
        CGRect stringRectWithTime = [tempMessageDataText boundingRectWithSize:CGSizeMake(maximumTextWidth, CGFLOAT_MAX)
                                                             options:(NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading)
                                                          attributes:@{ NSFontAttributeName : layout.messageBubbleFont }
                                                             context:nil];
        
//        NSLog(@"stringRect width : %f", stringRect.size.width);
        
        CGSize stringSize = CGRectIntegral(stringRect).size;
        CGSize stringSizeWithTime = CGRectIntegral(stringRectWithTime).size;
        
//        NSLog(@"*****stringSize height : %f", stringSize.height);
//        NSLog(@"*****stringSize width : %f", stringSize.width);
        
        int timeStampHeight = 0;   //talkmdev - define timeStamp height
        
        if (stringSize.height == stringSizeWithTime.height){   //talkmdev - control timestamp vertical height
            //this means [text + time stamp] and [text] height is same
            timeStampHeight = 0;  // timestamp will display behind the same line. so don add height
            contentStringSize = stringSizeWithTime;  //set content width will follow [text + time stamp]
        }
        else{
            timeStampHeight = 5;
            contentStringSize = stringSize;
        }
        
        CGFloat verticalContainerInsets = layout.messageBubbleTextViewTextContainerInsets.top + layout.messageBubbleTextViewTextContainerInsets.bottom + timeStampHeight;
        CGFloat verticalFrameInsets = layout.messageBubbleTextViewFrameInsets.top + layout.messageBubbleTextViewFrameInsets.bottom + timeStampHeight;
...
//        finalSize = CGSizeMake(finalWidth, stringSize.height + verticalInsets);
        ...
        CGFloat finalWidth = MAX(contentStringSize.width + horizontalInsetsTotal, self.minimumBubbleWidth) + self.additionalInset;

        finalSize = CGSizeMake(finalWidth, contentStringSize.height + verticalInsets);
    }
