...
@property (weak, nonatomic) IBOutlet JSQMessagesLabel *messageBubbleTimeStampLabel;
...
@property (weak, nonatomic) IBOutlet UIView *uiCustomView;

@end


@implementation JSQMessagesCollectionViewCell{
    UIView *topLabelBackgroundView;
}

int const topLabelHeight;
...
//
//    if (sdata.isDailySet == nil){
//        ////    talkmdev - daily toplabel background
//        [self.cellTopLabel layoutIfNeeded];
//        [topLabelBackgroundView removeFromSuperview];
//
//        topLabelBackgroundView = [[UIView alloc]init];
//        topLabelBackgroundView.frame = CGRectMake(self.cellTopLabel.frame.origin.x, self.cellTopLabel.frame.origin.y, self.cellTopLabel.intrinsicContentSize.width + 20, self.cellTopLabel.intrinsicContentSize.height);
//
//        //    topLabelBackgroundView.frame = CGRectMake(self.cellTopLabel.frame.origin.x, self.cellTopLabel.frame.origin.y, self.cellTopLabel.intrinsicContentSize.width + 20, 16);
//
//        topLabelBackgroundView.backgroundColor = HexColor(0x888888);
//        topLabelBackgroundView.alpha = 0.1;
//        topLabelBackgroundView.layer.cornerRadius = 6;
//
//        //    CGPoint center123 = CGPointMake(CGRectGetMidX(self.cellTopLabel.bounds), CGRectGetMidY(self.cellTopLabel.bounds));
//        CGPoint center123 = CGPointMake(WINDOW_WIDTH / 2, self.cellTopLabel.frame.size.height / 2);
//        topLabelBackgroundView.center = center123;
//        [self.cellTopLabel addSubview:topLabelBackgroundView];
//        ////    talkmdev - daily toplabel background
//
//        sdata.isDailySet = @"D";
//    }
...
//- (CGRect)boundingRectForCharacterRange:(NSString *)
//{
...
- (void)jsq_handleTapGesture:(UITapGestureRecognizer *)tap
{
    ...
    else if (CGRectContainsPoint(self.statusContainer.frame, touchPt)) {
        if (self.statusContainer.hidden == NO){
            [self.delegate messagesCollectionViewCellDidTapResend:self];
        }
    }
    else {
