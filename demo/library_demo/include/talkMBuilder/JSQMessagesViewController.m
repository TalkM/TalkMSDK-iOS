...
NSString * const titleResend = @"Resend";
NSString * const titleContentResend = @"Are you sure you want to resend this message?";
int const resendTag = 213;
...
//    self.collectionView.backgroundColor = HexColor(0xf5f6f1);
    self.collectionView.backgroundColor = HexColor(0xebebeb);  //talkmdev chat background color
    ...
    self.inputToolbar.contentView.backgroundColor = HexColor(0xffffff); //talkm toolbar
    self.inputToolbar.contentView.textView.placeHolder = [NSBundle jsq_localizedStringForKey:@"new_message"];
    
    self.inputToolbar.contentView.textView.layer.cornerRadius = 5;  //talkm txtReply
    
//    self.inputToolbar.contentView.textView.contentInset = UIEdgeInsetsMake(0.0f, 40.0f, 0.0f, 0.0f);;  //talkm txtReply
    ...
    //talkmdev put rating checking and display CellRating
//    ShareData *sdata = [ShareData ShareDataAction];
//    
//    if ([sdata.isRating isEqualToString:@"X"]){
//        self.incomingCellIdentifier = [CustomJSQMessagesCollectionViewCellIncoming cellReuseIdentifier];
//    }
//    else{
...
- (void)didPressSendButton2:(UIButton *)button
            isEmojiKeyboard:(BOOL *)isEmojiKeyboard
{
    NSAssert(NO, @"Error! required method not implemented in subclass. Need to implement %s", __PRETTY_FUNCTION__);
}
...
//    cell.cellTopLabel.layer.mask = maskLayer;
    ...
    //talkmdev - customize bubble status
    NSString *bottomText = [cell.cellBottomLabel.attributedText string];
    
    if (bottomText.length > 0 && [bottomText isEqualToString:@"Failed!"]){
        cell.statusContainer.hidden = NO;
        [cell.statusContainer setTag:resendTag];
        
        //move to messagesCollectionViewCellDidTapResend
//        UITapGestureRecognizer *tap1= [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(resendPressed:)];//your action selector
//        [tap1 setNumberOfTapsRequired:1];
//        [cell.statusContainer addGestureRecognizer:tap1];
        //move to messagesCollectionViewCellDidTapResend
        
        
        
        //cant hide the bottom label, so change its height instead
        //code change in ChatViewCOntroller.heightForCellBottomLabelAtIndexPath
//        cell.cellBottomLabel.attributedText = nil;   //hide the text
//        cell.cellBottomLabel.hidden = YES;
//        cell.cellBottomLabel.accessibilityFrame = CGRectZero;
        
    }
    else{
        cell.statusContainer.hidden = YES;
    }
    ...
    CGFloat width = cell.textView.intrinsicContentSize.width;
    
    if (isOutgoingMessage) {
        //talmdev - add sending status
//        JSQMessagesLabel *lbl = [[JSQMessagesLabel alloc] initWithFrame:CGRectMake(300, 0, 300, 100)];
//        lbl.text = @"test ni";
//        lbl.backgroundColor = [UIColor cyanColor];
//        [cell addSubview:lbl];
        cell.messageBubbleTopLabel.textInsets = UIEdgeInsetsMake(0.0f, 0.0f, 0.0f, bubbleTopLabelInset);
//        lbl.textInsets = UIEdgeInsetsMake(0.0f, 0.0f, 0.0f, bubbleTopLabelInset);
    }
    else {
        cell.messageBubbleTopLabel.textInsets = UIEdgeInsetsMake(0.0f, bubbleTopLabelInset, 0.0f, 0.0f);
//        lbl.textInsets = UIEdgeInsetsMake(0.0f, bubbleTopLabelInset, 200.0f, 30.0f);
    }
...
- (void)collectionView:(JSQMessagesCollectionView *)collectionView
 didTapCellAtIndexPath:(NSIndexPath *)indexPath
    ...
    NSLog(@"didTapCellAtIndexpath %i", indexPath.item);
}

#pragma mark - Input toolbar delegate

//-(void)messagesInputToolbar:(JSQMessagesInputToolbar *)toolbar setRightBarButtonItem2:(BOOL)isEmoji{
//
//}
...
- (void)messagesInputToolbar:(JSQMessagesInputToolbar *)toolbar didPressRightBarButton2:(UIButton *)sender
{
    [self didPressSendButton2:sender
              isEmojiKeyboard:self.isEmojiKeyboard];
}
...
- (void)keyboardController:(JSQMessagesKeyboardController *)keyboardController keyboardDidChangeFrame:(CGRect)keyboardFrame
{
    ...
    ////talkmdev - show power by
    //IMPORTANT : change toolbar bottom layout constrain from 0 to 30 20171129
//    ShareData *sdata = [ShareData ShareDataAction]; //talkmdev - powerby bottom constraint
//    if (heightFromBottom == 0){
//        heightFromBottom = [sdata.chatPowerByBottomConstraint integerValue];  //talkm - power by height
//    }
...
-(void)resendPressed:(id)sender{
    ...
    NSLog(@"text bubble press after end conv!!!");
    
//    int num = (int)(((UITapGestureRecognizer *)sender).view.tag);
//
//    if (num == resendTag){
    
    [self.inputToolbar.contentView.textView resignFirstResponder];
    
        dialogYesNo = [libsdk dialogAlert:self titleText:titleResend messageText:titleContentResend leftButtonText:@"Confirm" rightButtonText:@"Cancel" andCompletion:^(BOOL finished) {
            if (finished){
                NSLog(@"finisied resend");
            }
        }];
        
        CGPoint center123 = CGPointMake(CGRectGetMidX(self.view.bounds), CGRectGetMidY(self.view.bounds));
        dialogYesNo.center = center123;
        
        [self.view addSubview:dialogYesNo];
        ...
        [self.view bringSubviewToFront:dialogYesNo]; 

//    }
}

-(void)dialogYesNoLeftButtonAction:(id)sender{
    NSLog(@"callback success");
    
    [self dismissDialogYesNo];
    
//    [self backTapped:self];
}

-(void)dialogYesNoRightButtonAction:(id)sender{
    
    NSLog(@"callback right success");
    
    [self dismissDialogYesNo];
}

-(void)dismissDialogYesNo{
    [background removeFromSuperview];
    [dialogYesNo removeFromSuperview];
}
