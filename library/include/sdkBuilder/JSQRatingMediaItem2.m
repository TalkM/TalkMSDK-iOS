...
- (instancetype)initWithCustomView:(UIView *)ratingView msgType:(int)msgType bubbleColor:(NSString *)bubbleColor isRoundBubble:(BOOL)isRoundBubble isAdvertisement:(BOOL)isAdvertisement webHeight:(NSInteger)webHeight webWidth:(NSInteger)webWidth{
...
{
    if (_msgType == MsgType_RATE){
        return CGSizeMake(220.0f,  //width
                          42.0f   //height
                          );
    }
    else if(_msgType == MsgType_END_CONV){
        return CGSizeMake(300.0f,  //width
                          75.0f   //height
                          );
    }
    else if(_msgType == MsgType_END_CONV_RETURN){
        return CGSizeMake(300.0f,  //width
                          40.0f   //height
                          );
    }
    else if(_msgType == MsgType_COMP_ADV){
        if (_isAdvertisement){
...
//                              );
    ...
    else if (_msgType == MsgType_TRANSFER){
        return CGSizeMake(300,  //width
                          60   //height
                          );
    }
